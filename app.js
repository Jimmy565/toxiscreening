...
function inferRiskSignals(query, smiles = '', sourceCount = 0, sourceNames = []) {
  ...
  const hazardTriage = Math.min(92, 30 + sourceBoost + triggered.length * 8 + (haystack.length > 20 ? 8 : 0));
  ...
  return {
    hazardTriage: Math.round(hazardTriage),
...
function buildExpertSummary(query, sources, scores) {
  ...
  return {
    ...
    analysisProfiles: {
      ...
      toxicity: {
        label: 'ToxiScreen hazard triage',
        method: 'Structure/name alert screening and public toxicology context; not an official regulatory prediction.',
      },
    },
    notes: [
      describeRisk(scores.hazardTriage, 'Hazard triage'),
...
app.post('/api/predict', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      scores: riskSignals,
      dataSources,
      limitations: 'These are preliminary screening signals generated by ToxiScreen logic and are not a substitute for validated models or experimental testing.',
    });
  } catch (error) {
